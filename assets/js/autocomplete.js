function autocomplete(inp, arr) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          // if (arr[i].nome_ingrediente.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
        if (arr[i].nome_ingrediente.toLowerCase().includes(val.toLowerCase())) {
            /*create a DIV element for each matching element:*/
            b = document.createElement("DIV");
            /*make the matching letters bold:*/
            b.innerHTML = "<strong>" + arr[i].nome_ingrediente.substr(0, val.length) + "</strong>";
            b.innerHTML += arr[i].nome_ingrediente.substr(val.length);
            /*insert a input field that will hold the current array item's value:*/
            b.innerHTML += "<input type='hidden' value='" + JSON.stringify(arr[i]) + "'>";
            /*execute a function when someone clicks on the item value (DIV element):*/
            b.addEventListener("click", autoCompleteClickEvent);
            a.appendChild(b);
          }
        }
    });
